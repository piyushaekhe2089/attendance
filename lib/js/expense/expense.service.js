...
angular.
  module('expense').
  service('ExpenseService', function($q, $http, $localStorage){
	  var data = {};
	  var employees;  
	  ...
	  return {
		  ...
		  getClaims: function(){
			var claims_defer = $q.defer();
			  var req = {
				  method: 'GET',
				  url: baseURL + 'api/claim/',
				  //cache: false,
			  };

			  $http(req).then(function (response) {
				  console.log(response.data);
				  claims_defer.resolve(response.data);
			  }).catch(function onError(error) {
				  console.log(error);
			  });

			  return claims_defer.promise;
		  },
		  /*
