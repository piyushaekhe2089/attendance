		...
		controller: ['$http', '$stateParams', 'EmployeeService',
			function EmployeeEditController($http, $stateParams, EmployeeService) {
				...
				self.save = function save() {
					...
					var managers = [];
					for (var i = 0; i < self.employee.first_level_reporting.length; i++) {
						managers.push(self.employee.first_level_reporting[i].id)
					}
					self.employee['first_level_reporting_set'] = managers;

					var managers = [];
					for (var i = 0; i < self.employee.second_level_reporting.length; i++) {
						managers.push(self.employee.second_level_reporting[i].id)
					}
					self.employee['second_level_reporting_set'] = managers;
